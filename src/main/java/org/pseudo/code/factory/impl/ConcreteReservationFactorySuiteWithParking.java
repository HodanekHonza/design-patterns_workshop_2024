...
public class ConcreteReservationFactorySuiteWithParking implements ReservationFactory {
    @Override
    public Reservation createReservation(ReservationInfo info) {
        ...
        if (isRoomAvailable) {
            ...
            return new Reservation(info.getGuest(), "Suite", room, true, info.getStartDate(), info.getEndDate());
        } else {
            // Handle room unavailability (e.g., throw an exception or return null)
            throw new RoomUnavailableException("No suite with parking is available for the specified dates.");
        }
    }
