...
public class ConcreteReservationFactoryBasicWithParking implements ReservationFactory {
    @Override
    public Reservation createReservation(ReservationInfo info) {
        ...
        if (isRoomAvailable) {
            ...
            return new Reservation(info.getGuest(), "Basic", room, true, info.getStartDate(), info.getEndDate());
        } else {
            // Handle room unavailability (e.g., throw an exception or return null)
            throw new RoomUnavailableException("No basic room with parking is available for the specified dates.");
        }
    }
